...
public class MoniPrintListAdapter extends RecyclerView.Adapter<MoniPrintListAdapter.Holder> {

    final Context adapter_context;
    private List<MoniPrintList> list_moniprint;
    ...
    public MoniPrintListAdapter(Context adapter_context, List<MoniPrintList> list_moniprint, int layout_type, HomeFragment fragment_home) {
        this.adapter_context = adapter_context;
        this.list_moniprint = list_moniprint;
    ...
    public void onBindViewHolder(@NonNull Holder holder, int position) {
        holder.tv_title.setText(list_moniprint.get(position).get_name());
        holder.iv_icon.setImageResource(list_moniprint.get(position).get_icon());
        holder.cv_item_menu.setEnabled(list_moniprint.get(position).get_is_enabled());
            ...
            switch (getAdapterPosition()){
                default:
                    if (popup_size_quantity.check_sheet()) {
                        ...
                        data_sheet.putInt("icon", list_moniprint.get(getAdapterPosition()).get_icon());
                        data_sheet.putString("title", list_moniprint.get(getAdapterPosition()).get_name());
